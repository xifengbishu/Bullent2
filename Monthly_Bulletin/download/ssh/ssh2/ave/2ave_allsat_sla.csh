...
# === sla anomaly for 2010 - 2009 
  ...
  set month = $2
  set anomaly = $3
  set mete_root = /public/flow/WGS/Bulletin/Monthly_Bulletin/download/ssh/ssh2/ori_file

  mkdir temp
  rm -f temp/*
  
  ln -s $mete_root/nrt_global_merged_msla_h_${year}${month}*.nc ./temp
  cd temp
  set num = 1
  foreach file ( `ls *nc` )
   echo $file' '$num
   ncks -v Grid_0001 $file ww_$num.nc 
   set num = `expr $num + 1`
  end
  ncea ww_*nc ave_nrt_global_merged_msla_h_${year}${month}.nc
  mv ave_nrt_global_merged_msla_h_${year}${month}.nc ../
exit
  # ====== cha ====  
