...
* --- ctl name (numbers = y_num)
  anomaly = 1
  if ( anomaly = 1 ) 
    'sdfopen ave_ssh/cha_climate_y2015_m03.nc'
  else
    'sdfopen ave_ssh/ave_nrt_global_allsat_msla_h_201503.nc'
  endif
...
* ---  1: yes ; 0: no (default domain is used)
  reset_domain = 1
  lon_domain='lon 25 115'
  lat_domain='lat -35 25'
...
*      'set map 1 1 8'
    if ( anomaly = 1 ) 
      case_name='ssh_anomaly_'03
    else
      case_name='ssh_month_'03
    endif
*      'set mpdset hires'
'set map 1 1 5'
'set mpdset hires'
...
* --- longitude
  ...
  axis_unit = '`3.`n'
  xaxis_unit_end = '`3.`1E'
  yaxis_unit_end1= '`3.`1N'
  yaxis_unit_end2= '`3.`1S'
  while( x < x_end )
* ----------
    nx = 1
    xlen = xlon2 - xlon1
...
* ----------
    ...
    while( ny <= nylab )
     ...
     'set strsiz 'string_size
      if( ylat < 0 ) 
        ylats = ylat * (-1)
        'draw string 'x_lat-0.1' 'pos' 'ylats''yaxis_unit_end2''
      endif
      if( ylat = 0 )
*        'draw string 'x_lat-0.12' 'pos+0.08' EQ'
      ...
      if( ylat > 0 ) 
        if ( ny = nylab )
        'draw string 'x_lat-0.1' 'pos' 'ylat''yaxis_unit_end2''
*          'draw string 'x_lat-0.12' 'pos-0.05' 'ylat''axis_unit'N'
*          'draw string 'x_lat-0.18' 'pos+0.15' N'
        else
*          'draw string 'x_lat-0.1' 'pos' 'ylat''axis_unit''
           'draw string 'x_lat-0.1' 'pos' 'ylat''yaxis_unit_end1''
        endif
...
_nheight = '-50 -45 -40 -35 -30 -25 -20 -15 -10 -5 0 5 10 15 20 25 30 35 40 45 50' 
  endif
  if( color_num = 166 ) 
'set rgb 61 254 254 170'
'set rgb 60 238 254 254'
